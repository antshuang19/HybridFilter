...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    for i in range(ksize):
        kernel[i] = np.exp(-((i-mean)**2) / (2 * sigma **2))/ (np.sqrt(2 *np.pi) *mean) # base on pdf formula 
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    kernel = np.dot(create_Gaussian_kernel_1D(ksize,cutoff_frequency), create_Gaussian_kernel_1D(ksize,cutoff_frequency).T)
    kernel = kernel/np.sum(kernel)
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    a, b, c = image.shape
    k, j = filter.shape
    padding_image = np.pad(image, ((k // 2, k // 2), (j // 2, j // 2), (0, 0)), constant_values=0)
    filtered_image = np.zeros((a, b, c))
    for z in range(c):
        for A in range(a):
            for B in range(b):
                block = padding_image[A:A + k, B:B + j, z]
                filtered_image[A, B, z] = np.sum(np.multiply(block, filter))
    ### END OF STUDENT CODE ####
    ############################
